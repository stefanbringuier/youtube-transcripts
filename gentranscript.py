...
def parse_arguments():
    ...
    parser.add_argument('-o', '--output', type=str, default=None, help='Path for the output transcript file (default: transcript_[date])')
    ...
    parser.add_argument('-ai','--open-ai-summary',action="store_true",help="Summarize the transcript. Note OPENAI_API_KEY env variable should be set.")
...
def download_audio(yt, video_id, cache_folder):
    ...
    if not os.path.exists(audio_path):
        ...
        print(f"Finished downloading audio for video ID: {video_id}. Caching temporarily.")
    else:
...
def display_summary(summary,url):
    ...
    formatted_summary = Text("\n".join(str(line) for line in formatted_lines), justify="left", style=f"bg:{blended_color}")

    # Add padding and create a panel around the text
    padded_summary = Padding(formatted_summary, (1, 2))
    panel = Panel(padded_summary, expand=True, title=f"Summary for {url}", title_align="left")
